...
<div class="row-fluid show-for-medium-down">
    ...
    <div class="row-fluid clearfix panel">
        <div class="small-12 columns text-center"><?php echo Html::a($w->localUrl("/task/tasklist?assignee_id=" . $w->Auth->user()->id), (count($tasks) . " Task" . (count($tasks) == 1 ? "" : "s"))); ?> assigned to you</div>
        <div class="small-12">
            <div class="row-fluid clearfix">
                <?php if ($count_overdue > 0) : ?>
                    <div class="small-12 medium-6 columns text-center"><strong><?php echo Html::a("/task/tasklist?assignee_id=" . $w->Auth->user()->id . "&dt_to=" . formatDate(time(), "Y-m-d"), $count_overdue . " overdue"); ?></strong></div>
                <?php endif; ?>
                <?php if ($count_due_soon == 0) : ?>
                    <div class="small-12 medium-6 columns text-center"><b>0 due</b> within 7 days</div>
                <?php else: ?>
                    <div class="small-12 medium-6 columns text-center"><b><?php echo Html::a("/task/tasklist?assignee_id=" . $w->Auth->user()->id . "&dt_from=" . formatDate(time(), 'Y-m-d') . "&dt_to=" . formatDate((time() + (60 * 60 * 24 * 7)), "Y-m-d"), $count_due_soon . " due"); ?></b> within 7 days</div>
                <?php endif; ?>
                <?php if ($count_no_due_date > 0) : ?>
                    <div class="small-12 medium-6 columns text-center"><?php echo Html::a("/task/tasklist?assignee_id=" . $w->Auth->user()->id . "&dt_to=NULL", $count_no_due_date); ?> without a due date</div>
                <?php endif; ?>
                <?php if ($count_todo_urgent == 0) : ?>
                ...
                <?php else : ?>
                    <div class="small-12 medium-6 columns text-center"><?php echo Html::a($w->localUrl("/task/tasklist?assignee_id=" . $w->Auth->user()->id . "&task_priority=Urgent"), $count_todo_urgent . " task" . ($count_todo_urgent == 1 ? "" : "s")); ?> marked <strong>urgent</strong></div>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    <?php if (!empty($time_entries)) : ?>
        <?php foreach($time_entries as $date => $entry_struct) : ?>
            ...
            <table class='small-12'>
                <thead><tr><th width="10%">From</th><th width="10%">To</th><th width="20%">Task</th><th width="50%">Description</th><th>Actions</th></tr></thead>
                <tbody>
                    <?php foreach($entry_struct['entries'] as $time_entry) : ?>
                        <tr>
                            ...
                            <td><?php echo $time_entry->getTask()->toLink(); ?></td>
